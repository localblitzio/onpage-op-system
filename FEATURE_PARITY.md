...
- Cora Profiles: local and cloud now support create/read/update, client attach/detach, archive behavior, and embedded Domain List management for dashboard profile metadata. Native Windows Cora actions remain local-only and are queued from cloud through the bridge; next check is real bridge execution of apply/push commands.
- Cora Domain Lists: local and cloud now support synced CRUD/archive for tracked domains, competitors, banned domains, slow-render domains, and stop words inside the Cora Profiles workflow. Native Windows Cora apply/pull actions are queued from cloud through the bridge; next check is real bridge execution.
